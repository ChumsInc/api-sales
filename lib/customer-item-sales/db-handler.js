...
function dataKey({ ARDivisionNo, CustomerNo, ShipToCode, ItemCode, FiscalCalYear }) {
    return [ARDivisionNo, CustomerNo, ShipToCode, ItemCode, FiscalCalYear].join('-');
}
export async function loadCustomerItemSalesV2({ ARDivisionNo, CustomerNo, ItemCode, FiscalCalYear, groupShipTo, userId }) {
    try {
        const sql = `
