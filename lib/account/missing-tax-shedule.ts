...
import {MissingTaxSchedule} from "./types.js";

const debug = Debug('chums:lib:account:missing-tax-schedule');

type MissingTaxScheduleRow = RowDataPacket & MissingTaxSchedule;
...
export async function loadMissingTaxSchedules():Promise<MissingTaxSchedule[]> {
    try {
        const [rows] = await mysql2Pool.query<MissingTaxScheduleRow[]>(sql);
        return rows;
    } catch(err) {
