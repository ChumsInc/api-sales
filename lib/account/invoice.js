...
import { apiFetchJSON, mysql2Pool, validateUserAccount } from 'chums-local-modules';
...
async function loadInvoice({ InvoiceNo, userId, includeDetail, }) {
    try {
        ...
        if (!invoice) {
            ...
            const json = await apiFetchJSON(`https://intranet.chums.com/node-sage/api/CHI/invoice/${InvoiceNo}`);
            return json.result;
        }
