...
const qryInvoicedP1 = `
SELECT h.Company,
       ifnull(i.Category4, 'NULL') as Category4, 
...
const query = `
    SELECT ifnull(nullif(d.Category4, ''), 'N/A')                 AS key_field,
    ...
    FROM (SELECT DISTINCT i.Category4 FROM c2.ci_item i where Company = :company) d
         INNER JOIN ( $SUB_QUERIES$ ) h
                    ON h.Category4 = d.Category4 
    GROUP BY d.Category4
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
