...
import { joinedTables, customerFilters, invoiceFilters, itemFilters, openOrderFilters } from './query-snippets.js';
const qryInvoicedP1 = `
    SELECT h.Company,
           d.ItemCode,
...
`;
const query = `
    SELECT h.ItemCode                                                            AS key_field,
           IFNULL(d.ItemCodeDesc, 'N/A')                                         AS ItemCodeDesc,
    ...
    FROM ($SUB_QUERIES$) h
             LEFT JOIN c2.ci_item d
                       ON h.Company = d.Company AND h.ItemCode = d.ItemCode
             LEFT JOIN (SELECT Company, ItemCode, GROUP_CONCAT(DISTINCT ItemStatus) AS ProductStatus
                        FROM c2.IM_ItemWarehouseAdditional
                        WHERE IFNULL(ItemStatus, '') <> ''
                        GROUP BY Company, ItemCode) ps ON ps.Company = h.Company AND ps.ItemCode = h.ItemCode
    GROUP BY key_field, ItemCodeDesc
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
