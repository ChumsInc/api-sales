...
const qryInvoicedP1 = `
    SELECT h.Company,
           IFNULL(im.PrimaryVendorNo, 'NULL')                 AS PrimaryVendorNo,
    ...
    WHERE h.Company = :company
      AND h.InvoiceDate BETWEEN :p1min AND :p1max
        ...
        ${customerFilters}
        ${itemFilters}
    GROUP BY Company, PrimaryVendorNo
`;
const qryInvoicedP2 = `
SELECT h.Company,
       ifnull(im.PrimaryVendorNo, 'NULL') as PrimaryVendorNo,
...
`;
const query = `
    SELECT ifnull(nullif(h.PrimaryVendorNo, ''), 'N/A')                 AS key_field,
           ifnull(d.VendorName, 'N/A') as VendorName,
    ...
    FROM ( $SUB_QUERIES$ ) h
         LEFT JOIN c2.AP_Vendor d
                    ON h.Company = d.Company and h.PrimaryVendorNo = d.VendorNo 
    GROUP BY key_field
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
