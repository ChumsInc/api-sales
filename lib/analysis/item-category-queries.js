...
const qryInvoicedP1 = `
    SELECT h.Company,
           ifnull(im.Category2, 'NULL')                       as category2,
...
const query = `
    SELECT ifnull(nullif(h.Category2, ''), 'N/A')                                AS key_field,
           ifnull(nullif(c.description, ''), h.Category2)                        as description,
    ...
    FROM ($SUB_QUERIES$) h
             LEFT JOIN c2.SKU_Categories c on c.code = h.Category2
    GROUP BY ifnull(nullif(h.Category2, ''), 'N/A')
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
