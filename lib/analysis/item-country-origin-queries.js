...
const qryInvoicedP1 = `
SELECT h.Company,
       ifnull(i.UDF_COUNTRY_ORIGIN, 'NULL') as UDF_COUNTRY_ORIGIN, 
...
WHERE h.Company = :company
  ...
  ${customerFilters}
  ${itemFilters}
GROUP BY Company, UDF_COUNTRY_ORIGIN
`;
...
const query = `
    SELECT ifnull(nullif(d.UDF_COUNTRY_ORIGIN, ''), 'N/A')                 AS key_field,
    ...
    FROM (SELECT DISTINCT i.UDF_COUNTRY_ORIGIN FROM c2.ci_item i where Company = :company) d
         INNER JOIN ( $SUB_QUERIES$ ) h
                    ON h.UDF_COUNTRY_ORIGIN = d.UDF_COUNTRY_ORIGIN 
    GROUP BY d.UDF_COUNTRY_ORIGIN
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
