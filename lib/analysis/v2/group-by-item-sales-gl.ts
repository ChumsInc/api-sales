...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Sales AS (SELECT IFNULL(id.SalesAcctKey, 'N/A')                                                       AS AccountKey,
                   ...
                   GROUP BY IFNULL(id.SalesAcctKey, 'N/A')),
         OpenOrders AS (SELECT IFNULL(d.SalesAcctKey, 'N/A') AS AccountKey,
                               SUM(
                        ...
                        GROUP BY IFNULL(d.SalesAcctKey, 'N/A')),
...
export async function loadItemSalesGLResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SAGLRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemSalesGLResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemSalesGLResults()", err);
        return {error: 'Unknown error in loadItemSalesGLResults', query, rows: []}
    }
}
