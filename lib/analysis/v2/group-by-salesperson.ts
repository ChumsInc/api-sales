import {SAParams, SAQueryResult, SARow, SASalesRepRow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:v2:group-by-salesperson");
...
export const sql = `
    ...
    SELECT CONCAT_WS('-', rep.SalespersonDivisionNo, IFNULL(rep.SalespersonNo, '--unassigned--')) AS key_field,
           rep.SalespersonName,
    ...
    FROM Customers d
             ...
             LEFT JOIN c2.ar_salesperson rep
                       ON rep.SalespersonDivisionNo = IFNULL(ShipTo.SalespersonDivisionNo, c.SalespersonDivisionNo) AND
                          rep.SalespersonNo = IFNULL(ShipTo.SalespersonNo, c.SalespersonNo)
    ...
    GROUP BY CONCAT_WS('-', rep.SalespersonDivisionNo, IFNULL(rep.SalespersonNo, '--unassigned--')),
             rep.SalespersonName
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SASalesRepRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SASalesRepRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadSalespersonResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SASalesRepRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadSalespersonResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadSalespersonResults()", err);
        return {error: 'Unknown error in loadSalespersonResults', query, rows: []}
    }
}
