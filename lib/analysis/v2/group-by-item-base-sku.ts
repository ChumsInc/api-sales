import {SADescribedRow, SAParams, SAQueryResult, SARow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:v2:group-by-item-base-sku");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   ...
                   FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.Category4, ''), 'N/A')   AS key_field,
           IFNULL(b.description, 'N/A')             AS description,
    ...
    FROM Items d
             ...
             LEFT JOIN c2.sku_bases b ON b.sku = i.Category4
    ...
    GROUP BY IFNULL(NULLIF(i.Category4, ''), 'N/A'),
...
async function loadAnalysis(params: SAParams): Promise<SARow<SADescribedRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SADescribedRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadBaseSKUResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SADescribedRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadBaseSKUResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadBaseSKUResults()", err);
        return {error: 'Unknown error in loadBaseSKUResults', query, rows: []}
    }
}
