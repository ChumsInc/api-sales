import {SAParams, SAQueryResult, SARow, SASelfDescribedRow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:v2:group-by-item-country");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   ...
                   FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.UDF_COUNTRY_ORIGIN, ''), 'N/A') AS key_field,
           IFNULL(cc.CountryName, '')                      AS description,
    ...
    FROM Items d
             ...
             LEFT JOIN c2.SY_Country cc ON cc.CountryCode = i.UDF_COUNTRY_ORIGIN
    WHERE (Sales.ItemCode IS NOT NULL
        OR OpenOrders.ItemCode IS NOT NULL)
    GROUP BY IFNULL(NULLIF(i.UDF_COUNTRY_ORIGIN, ''), 'N/A'),
             IFNULL(cc.CountryName, '')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SASelfDescribedRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SASelfDescribedRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadItemCountryResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SASelfDescribedRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemCountryResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemCountryResults()", err);
        return {error: 'Unknown error in loadItemCountryResults', query, rows: []}
    }
}
