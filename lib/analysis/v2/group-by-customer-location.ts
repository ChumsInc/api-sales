import {SALocationRow, SAParams, SAQueryResult, SARow} from "../sa-types.js";
...
export const sql = `
    ...
    SELECT IFNULL(CONCAT_WS('/', IFNULL(c.CountryCode, d.CountryCode), IFNULL(s.StateCode, d.State)),
                  'N/A')                                     AS key_field,
           IFNULL(IFNULL(s.StateCode, d.State), '-')         AS StateCode,
           IFNULL(IFNULL(c.CountryCode, d.CountryCode), '-') AS CountryCode,
           s.StateName,
           c.CountryName,
    ...
    FROM c2.ar_customer d
             LEFT JOIN c2.SY_Country c ON d.CountryCode = c.CountryCode
             LEFT JOIN c2.SY_State s ON d.State = s.StateCode AND d.CountryCode = s.CountryCode
    ...
    GROUP BY IFNULL(CONCAT_WS('/', IFNULL(c.CountryCode, d.CountryCode), IFNULL(s.StateCode, d.State)), 'N/A'),
             IFNULL(IFNULL(s.StateCode, d.State), '-'),
             IFNULL(IFNULL(c.CountryCode, d.CountryCode), '-'),
             s.StateName,
             c.CountryName
    ORDER BY :orderBy
    LIMIT :limit;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SALocationRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SALocationRow>[]>(getParsedSQL(params), params as QueryValues);
        await connection.query(sqlCleanup);
        await connection.end();
        return buildResult(results, params);
    } catch (err: unknown) {
        if (err instanceof Error) {
            debug("loadCustomerAnalysis()", err.message);
            return Promise.reject(err);
        }
        debug("loadCustomerAnalysis()", err);
        return Promise.reject(new Error('Error in loadCustomerAnalysis()'));
    } finally {
...
export async function loadCustomerLocationResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SALocationRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadCustomerLocationResults()", err.message);
