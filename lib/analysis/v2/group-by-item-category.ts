...
const debug = Debug("chums:lib:analysis:v2:group-by-item-category");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   ...
                   FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.Category2, ''), 'N/A')   AS key_field,
           IFNULL(x.description, 'N/A')             AS description,
    ...
    FROM Items d
             ...
             LEFT JOIN c2.SKU_Categories x ON x.code = i.Category2
    ...
    GROUP BY IFNULL(NULLIF(i.Category2, ''), 'N/A'),
           IFNULL(x.description, 'N/A')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
export async function loadItemCategoryResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SADescribedRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemCategoryResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemCategoryResults()", err);
        return {error: 'Unknown error in loadItemCategoryResults', query, rows: []}
    }
}
