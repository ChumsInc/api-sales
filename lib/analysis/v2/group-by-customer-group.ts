...
const debug = Debug("chums:lib:analysis:v2:group-by-customer-group");
...
export const sql = `
    ...
    SELECT IFNULL(NULLIF(d.SortField, ''), 'N/A')            AS key_field,
           IFNULL(NULLIF(d.SortField, ''), '--unassigned--') AS description,
    ...
    GROUP BY IFNULL(NULLIF(d.SortField, ''), 'N/A'),
           IFNULL(NULLIF(d.SortField, ''), '--unassigned--')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
export async function loadAnalysis(params: SAParams): Promise<SARow<SADescribedRow>[]> {
...
export async function loadCustomerGroupResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SADescribedRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadCustomerGroupResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadCustomerGroupResults()", err);
        return {error: 'Unknown error in loadCustomerGroupResults', query, rows: []}
    }
}
