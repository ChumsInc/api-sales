import {SAParams, SAQueryResult, SARow, SAGLRow} from "../sa-types.js";
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Sales AS (SELECT IFNULL(id.CostOfGoodsSoldAcctKey, 'N/A')                                                       AS AccountKey,
                   ...
                   GROUP BY IFNULL(id.CostOfGoodsSoldAcctKey, 'N/A')),
         OpenOrders AS (SELECT IFNULL(d.CostOfGoodsSoldAcctKey, 'N/A') AS AccountKey,
                               SUM(
                        ...
                        GROUP BY IFNULL(d.CostOfGoodsSoldAcctKey, 'N/A')),
         Accounts AS (SELECT DISTINCT AccountKey
                      FROM Sales
                      UNION
                      SELECT DISTINCT AccountKey
                      FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.Account, ''), 'N/A')     AS key_field,
           IFNULL(i.AccountDesc, '--unassigned--')                AS AccountDesc,
    ...
    FROM Accounts d
             LEFT JOIN c2.gl_account i ON i.AccountKey = d.AccountKey
             LEFT JOIN Sales ON d.AccountKey = Sales.AccountKey
             LEFT JOIN OpenOrders ON d.AccountKey = OpenOrders.AccountKey
    WHERE (Sales.AccountKey IS NOT NULL
        OR OpenOrders.AccountKey IS NOT NULL)
    GROUP BY IFNULL(NULLIF(i.Account, ''), 'N/A'),
             IFNULL(i.AccountDesc, '--unassigned--')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SAGLRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SAGLRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadItemCostGLResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SAGLRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemCostGLResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemCostGLResults()", err);
        return {error: 'Unknown error in loadItemCostGLResults', query, rows: []}
    }
}
