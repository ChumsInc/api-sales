...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
        ...
        Customers as (
            SELECT ARDivisionNo, CustomerNo, '' AS ShipToCode
            FROM c2.TEMP_SA_Customers
            UNION
            SELECT ARDivisionNo, CustomerNo, ShipToCode
            FROM c2.TEMP_SA_ShipTo
        )
    SELECT CONCAT_WS('-', d.ARDivisionNo, d.CustomerNo, ShipTo.ShipToCode) AS key_field,
           d.ARDivisionNo,
           d.CustomerNo,
           c.CustomerName,
           ShipTo.ShipToCode,
           IFNULL(ShipTo.ShipToName, c.CustomerName)                       AS ShipToName,
...
export async function loadShipToResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SACustomerRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadShipToResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadShipToResults()", err);
        return {error: 'Unknown error in loadShipToResults', query, rows: []}
    }
}
