...
const debug = Debug("chums:lib:analysis:v2:group-by-item-collection");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   ...
                   FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.Category3, ''), 'N/A')   AS key_field,
    ...
    GROUP BY IFNULL(NULLIF(i.Category3, ''), 'N/A')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
export async function loadItemCollectionResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SASelfDescribedRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemCollectionResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemCollectionResults()", err);
        return {error: 'Unknown error in loadItemCollectionResults', query, rows: []}
    }
}
