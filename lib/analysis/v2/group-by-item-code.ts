import {SAItemRow, SAParams, SAQueryResult, SARow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:v2:group-by-item-code");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Sales AS (SELECT id.ItemType,
                   ...
                   GROUP BY id.ItemType, id.ItemCode),
         OpenOrders AS (SELECT d.ItemType,
                               d.ItemCode,
                               SUM(
                        ...
                        GROUP BY d.ItemType, d.ItemCode),
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   FROM Sales
                   UNION
                   SELECT DISTINCT ItemType, ItemCode
                   FROM OpenOrders)
    SELECT d.ItemCode                          AS key_field,
           IFNULL(i.ItemCodeDesc, 'N/A')       AS ItemCodeDesc,
           IFNULL(ps.ProductStatus, '')        AS ProductStatus,
           IFNULL(Sales.p1_shipped, 0)         AS p1_shipped,
           0                                   AS p1_discount,
           IFNULL(Sales.p1_sales, 0)           AS p1_sales,
           IFNULL(Sales.p1_cogs, 0)            AS p1_cogs,
           IFNULL(OpenOrders.p1_open_qty, 0)   AS p1_open,
           IFNULL(OpenOrders.p1_open_sales, 0) AS p1_open_sales,
           IFNULL(OpenOrders.p1_open_cost, 0)  AS p1_open_cogs,
           IFNULL(Sales.p2_shipped, 0)         AS p2_shipped,
           0                                   AS p2_discount,
           IFNULL(Sales.p2_sales, 0)           AS p2_sales,
           IFNULL(Sales.p2_cogs, 0)            AS p2_cogs,
           IFNULL(OpenOrders.p2_open_qty, 0)   AS p2_open,
           IFNULL(OpenOrders.p2_open_sales, 0) AS p2_open_sales,
           IFNULL(OpenOrders.p2_open_cost, 0)  AS p2_open_cogs
    FROM Items d
             LEFT JOIN c2.CI_Item i ON i.ItemCode = d.ItemCode AND i.ItemType = d.ItemType
             LEFT JOIN Sales ON d.ItemType = Sales.ItemType AND d.ItemCode = Sales.ItemCode
             LEFT JOIN OpenOrders ON d.ItemType = OpenOrders.ItemType AND d.ItemCode = OpenOrders.ItemCode
             LEFT JOIN (SELECT ItemCode,
                               GROUP_CONCAT(DISTINCT CONCAT_WS(':', WarehouseCode, ItemStatus)) AS ProductStatus
                        FROM c2.IM_ItemWarehouseAdditional
                        WHERE IFNULL(ItemStatus, '') <> ''
                        GROUP BY ItemCode) ps ON ps.ItemCode = d.ItemCode
    WHERE (Sales.ItemCode IS NOT NULL
        OR OpenOrders.ItemCode IS NOT NULL)
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SAItemRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SAItemRow>[]>(getParsedSQL(params), params as QueryValues);
...
function getCustomerAnalysisQuery(options: SAParams): string {
    const _sql = [
        sqlCustomers,
        sqlItems,
        sqlGLAccounts,
...
export async function loadItemCodeResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SAItemRow>> {
    const query = getCustomerAnalysisQuery(params);
    if (skipExec) {
        return {rows: [], query}
    }
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemCodeResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemCodeResults()", err);
        return {error: 'Unknown error in loadItemCodeResults', query, rows: []}
    }
}
