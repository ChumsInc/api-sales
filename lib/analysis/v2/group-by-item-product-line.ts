...
const debug = Debug("chums:lib:analysis:v2:group-by-item-product-line");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   ...
                   FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.ProductLine, ''), 'N/A') AS key_field,
           IFNULL(pl.ProductLineDesc, 'N/A')        AS ProductLineDesc,
    ...
    FROM Items d
             ...
             LEFT JOIN c2.IM_ProductLine pl ON pl.ProductLine = i.ProductLine
    ...
    GROUP BY IFNULL(NULLIF(i.ProductLine, ''), 'N/A'),
             IFNULL(pl.ProductLineDesc, 'N/A')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SAProductLineRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SAProductLineRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadProductLineResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SAProductLineRow>> {
