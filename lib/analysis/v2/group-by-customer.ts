import {SACustomerRow, SAParams, SAQueryResult, SARow} from "../sa-types.js";
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         OpenOrders AS (SELECT h.ARDivisionNo,
                        ...
                        WHERE (h.ShipExpireDate BETWEEN :p1min AND :p1max
                          ...
                          AND (IFNULL(:ShipToCode, '') = ''
                            OR EXISTS (SELECT 1 FROM TEMP_SA_ShipTo WHERE ARDivisionNo = h.ARDivisionNo
                                                                      AND CustomerNo = h.CustomerNo
    ...
    SELECT CONCAT_WS('-', d.ARDivisionNo, d.CustomerNo) AS key_field,
           d.ARDivisionNo,
           d.CustomerNo,
           d.CustomerName,
...
async function loadAnalysis(params: SAParams): Promise<SARow<SACustomerRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SACustomerRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadCustomerResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SACustomerRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadCustomerResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadCustomerResults()", err);
        return {error: 'Unknown error in loadCustomerResults', query, rows: []}
    }
}
