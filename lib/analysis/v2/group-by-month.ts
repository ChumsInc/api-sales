import {SAMonthRow, SAParams, SAQueryResult, SARow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:v2:group-by-month");

export const sql = `
    WITH RECURSIVE
        ProductStatus AS (SELECT ItemCode, WarehouseCode
                          FROM c2.IM_ItemWarehouseAdditional
                          WHERE company = 'chums'
                            AND ItemStatus REGEXP :ProductStatus),
        Sales AS (SELECT MONTH(ih.InvoiceDate)                                                 AS month_number,
        ...
        Discounts AS (SELECT MONTH(ih.InvoiceDate) AS month_number,
                             SUM(CASE
                      ...
                      GROUP BY MONTH(ih.InvoiceDate)),
        OpenOrders AS (SELECT MONTH(h.ShipExpireDate) AS month_number,
                              SUM(
                       ...
                       GROUP BY MONTH(h.ShipExpireDate)),
        Months AS (SELECT 1 AS month_number
                   UNION ALL
                   SELECT month_number + 1
                   FROM Months
                   WHERE month_number < 12)
    SELECT d.month_number                               AS key_field,
           MONTHNAME(STR_TO_DATE(d.month_number, '%m')) AS MonthName,
           IFNULL(Sales.p1_shipped, 0)                  AS p1_shipped,
           IFNULL(Discounts.p1_discount, 0)             AS p1_discount,
           ...
           IFNULL(Discounts.p2_discount, 0)             AS p2_discount,
    ...
    FROM Months d
             LEFT JOIN Sales ON d.month_number = Sales.month_number
             LEFT JOIN Discounts ON d.month_number = Discounts.month_number
             LEFT JOIN OpenOrders ON d.month_number = OpenOrders.month_number
    WHERE (Sales.month_number IS NOT NULL
        OR Discounts.month_number IS NOT NULL
        OR OpenOrders.month_number IS NOT NULL)
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SAMonthRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SAMonthRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadMonthResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SAMonthRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadMonthResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadMonthResults()", err);
        return {error: 'Unknown error in loadMonthResults', query, rows: []}
    }
}
