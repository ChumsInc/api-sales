import {SAParams, SAQueryResult, SARow, SAVendorRow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:v2:group-by-item-vendor");
...
export const sql = `
    WITH ProductStatus AS (SELECT ItemCode, WarehouseCode
         ...
         Items AS (SELECT DISTINCT ItemType, ItemCode
                   ...
                   FROM OpenOrders)
    SELECT IFNULL(NULLIF(i.PrimaryVendorNo, ''), 'N/A') AS key_field,
           IFNULL(v.VendorName, '')                     AS VendorName,
    ...
    FROM Items d
             ...
             LEFT JOIN c2.AP_Vendor v ON v.VendorNo = i.PrimaryVendorNo
    WHERE (Sales.ItemCode IS NOT NULL
        OR OpenOrders.ItemCode IS NOT NULL)
    GROUP BY IFNULL(NULLIF(i.PrimaryVendorNo, ''), 'N/A'),
             IFNULL(v.VendorName, '')
    ORDER BY :orderBy
    LIMIT :limit;
`;
...
async function loadAnalysis(params: SAParams): Promise<SARow<SAVendorRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SAVendorRow>[]>(getParsedSQL(params), params as QueryValues);
...
export async function loadItemVendorResults(params: SAParams, skipExec?: boolean): Promise<SAQueryResult<SAVendorRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadItemVendorResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadItemVendorResults()", err);
        return {error: 'Unknown error in loadItemVendorResults', query, rows: []}
    }
}
