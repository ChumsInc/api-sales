import {SADivisionRow, SAParams, SAQueryResult, SARow} from "../sa-types.js";
...
const debug = Debug("chums:lib:analysis:division_v2");
...
export const sql = `
         ...
         Sales AS (SELECT ih.ARDivisionNo,
                   ...
                   WHERE (ih.InvoiceDate BETWEEN :p1min AND :p1max
                     ...
                     AND (IFNULL(:ShipToCode, '') = ''
                       OR EXISTS (SELECT 1 FROM TEMP_SA_ShipTo WHERE ARDivisionNo = ih.ARDivisionNo
                                                                 AND CustomerNo = ih.CustomerNo
    ...
    FROM c2.ar_division d
             LEFT JOIN Sales ON d.ARDivisionNo = Sales.ARDivisionNo
             LEFT JOIN Discounts ON d.ARDivisionNo = Discounts.ARDivisionNo
             LEFT JOIN OpenOrders ON d.ARDivisionNo = OpenOrders.ARDivisionNo
...
async function loadAnalysis(params: SAParams): Promise<SARow<SADivisionRow>[]> {
    ...
    try {
        ...
        const [results] = await connection.query<SARow<SADivisionRow>[]>(sql, params as QueryValues);
...
export async function loadARDivisionResults(params:SAParams, skipExec?: boolean): Promise<SAQueryResult<SADivisionRow>> {
    ...
    try {
        ...
        if (err instanceof Error) {
            console.debug("loadARDivisionResults()", err.message);
            return {error: err.message, query, rows: []}
        }
        console.debug("loadARDivisionResults()", err);
        return {error: 'Unknown error in loadARDivisionResults', query, rows: []}
    }
}
