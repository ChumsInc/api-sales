...
const query = `
    SELECT ifnull(nullif(concat_ws('/', nullif(h.ShipToCountryCode, ''), nullif(h.ShipToState, '')), ''), 'N/A')                                                        AS key_field,
           ifnull(nullif(h.ShipToState, ''), '-') as StateCode,
           ifnull(nullif(h.ShipToCountryCode, ''), '-') as CountryCode,
           d.StateName,
           c.CountryName,
    ...
    FROM ( $SUB_QUERIES$ ) h
        LEFT JOIN c2.SY_State d
            ON d.CountryCode = h.ShipToCountryCode
            AND d.StateCode = h.ShipToState
        LEFT JOIN c2.SY_Country c on c.CountryCode = h.ShipToCountryCode
    GROUP BY h.ShipToCountryCode, h.ShipToState
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
