...
export function parseRequestParams({ query, body }) {
    const { ARDivisionNo, CustomerNo = '', ShipToCode, created_min: minCreatedDate, created_max: maxCreatedDate, SalespersonNo, CustomerType, CustomerGroup, State, CountryCode, ShipToState, SalesAccount, CostAccount, ProductLine, Category2, Category3, BaseSKU, ItemCode, ProductStatus, PrimaryVendorNo, countryOfOrigin, p1min, p1max, p2min, p2max, limit, method, 'include-open-orders': openorders, 'include-discounts': discounts, sort, strictFilter } = { ...body, ...query };
