...
/**
 *
 * @param {function|string} queryFunction
 * @param {object} params
...
async function loadResults(queryFunction, params) {
    try {
        ...
        const query = typeof queryFunction === 'function' ? queryFunction(params) : queryFunction;
...
async function buildSQL(params) {
    ...
    return Promise.reject(`Query not defined: ${params.method}`);
}
 ...
 */
export async function execQuery(params) {
    try {
        switch (params.method) {
        case 'ARDivisionNo':
        ...
        case 'CustomerGroup':
            return await loadResults(customerGroup.buildQuery, params);
        case 'month':
        ...
        debug("execQuery()", params.method, err.message);
        return Promise.reject(err);
    }
}
...
export async function getSalesAnalysis(req, res) {
    ...
    try {
        ...
        const sql = await buildQuery(params);
        const query = parseSQL(sql, params) ?? '';
        res.status(500).json({error: err.message, params, query: query});
    }
}
