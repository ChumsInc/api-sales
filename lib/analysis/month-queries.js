...
import { customerFilters, invoiceDiscountFilters, invoiceFilters, itemFilters, joinedTables, openOrderFilters } from './query-snippets.js';
const qryInvoicedP1 = `
    SELECT h.Company,
...
const qryOpenP1 = `
    SELECT h.Company,
           month(h.ShipExpireDate)                                                  as month,
           0                                                                        AS p1_shipped,
           0                                                                        AS p1_sales,
           0                                                                        AS p1_cogs,
           0                                                                        AS p1_disc,
...
const query = `
    ...
    FROM (SELECT DISTINCT Company,
          ...
          WHERE Company = :company
            AND ARDivisionNo <> '00') d
             LEFT JOIN ($SUB_QUERIES$) h
                       ON h.Company = d.Company
                           AND h.month = d.month
    GROUP BY d.Company, d.month
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
