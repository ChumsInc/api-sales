...
const qryInvoicedP1 = `
SELECT h.Company,
       ifnull(i.Category3, 'NULL') as Category3, 
...
const query = `
    SELECT ifnull(nullif(d.Category3, ''), 'N/A')                 AS key_field,
    ...
    FROM (SELECT DISTINCT i.Category3 FROM c2.ci_item i where Company = :company) d
         INNER JOIN ( $SUB_QUERIES$ ) h
                    ON h.Category3 = d.Category3 
    GROUP BY d.Category3
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
