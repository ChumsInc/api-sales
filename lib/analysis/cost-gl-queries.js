...
const qryInvoicedP1 = `
    ...
    GROUP BY Company, CostOfGoodsSoldAcctKey
`;
const qryInvoicedP2 = `
    SELECT h.Company,
           d.CostOfGoodsSoldAcctKey,
...
`;
const query = `
    SELECT IFNULL(NULLIF(IFNULL(d.Account, h.CostOfGoodsSoldAcctKey), ''), 'N/A') AS key_field,
           d.AccountDesc,
    ...
    FROM ($SUB_QUERIES$) h
             LEFT JOIN c2.gl_account d
                       ON h.Company = d.Company
                           AND h.CostOfGoodsSoldAcctKey = d.AccountKey
    GROUP BY h.Company, h.CostOfGoodsSoldAcctKey
    ORDER BY $ORDER_BY$
    LIMIT :limit
`;
