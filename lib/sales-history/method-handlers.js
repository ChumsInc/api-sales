...
export async function getMonthsTotals(req, res) {
    try {
...
export async function getMonthOpenTotals(req, res) {
    try {
...
export async function getHistoryGraphTotals(req, res) {
    const userId = res.locals.profile?.user?.id ?? 0;
    const [goalBudget, history, openOrders] = await Promise.all([loadCompanyGoal(), loadMonthHistory(userId), loadMonthOpen(userId)]);
