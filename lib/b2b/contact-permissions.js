...
const debug = Debug('chums:lib:b2b:contact-permissions');
...
export async function postContactPermissions(req, res, next) {
    try {
...
export async function getContactPermissions(req, res) {
    try {
        let params = req.params;
        const [result = {...defaultPermissions}] = await loadUserPrivilege(params);
        res.json({result});
