...
async function loadOpenOrderDetail(salesOrderNo) {
    try {
        const sql = `
            SELECT d.LineKey,
                   d.LineSeqNo,
...
async function loadOpenOrders({ arDivisionNo, customerNo, salesOrderNo, userId }) {
    try {
...
export const getOpenSalesOrder = async (req, res) => {
    try {
        const [arDivisionNo, customerNo] = (req.params.customerKey ?? '').split('-');
        const params = {
            arDivisionNo,
