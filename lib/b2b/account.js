...
async function loadCustomerUsers({Company, ARDivisionNo, CustomerNo, isRepAccount = false, userid = 0}) {
    try {
        ...
        const query = `SELECT distinct ua.id, ua.name, ua.email, ua.accountType, 1 as billTo, null as shipToCode
                       ...
                       from users.user_SO_ShipToAddress sta
                                ...
                                inner join users.accounts uaa on uaa.id = sta.accountAccessId
                       WHERE cua.userid = :userid
                         AND cua.Company = :Company
                         AND cua.ARDivisionNo = :ARDivisionNo
                         AND cua.CustomerNo = :CustomerNo
                         AND ua.active = 1
                         and ua.accountType >= cu.accountType
                         AND (uaa.isRepAccount = 0 OR ua.accountType > cu.accountType OR uaa.userid = :userid)
                         AND (uaa.ShipToCode is not null or uaa.userid = :userid)

                       GROUP BY id, name, email, accountType`;
        const data = {Company, ARDivisionNo, CustomerNo, userid};
