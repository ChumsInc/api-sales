 ...
 * @return {Promise<*>}
 */
async function loadLoggedOpenOrders({arDivisionNo, customerNo, orderType, salesOrderNo}) {
    try {
        ...
        return rows.map(row => {
            ...
            try {
                ...
                debug("loadLoggedOrders()", err.message, row.SalesOrderNo);
        ...
        if (err instanceof Error) {
            debug("loadLoggedOpenOrders()", err.message);
            return Promise.reject(err);
        }
        debug("loadLoggedOpenOrders()", err);
        return Promise.reject(new Error('Error in loadLoggedOpenOrders()'));
    }
}
 ...
 * @return {Promise<*>}
 */
async function loadLoggedSaleOrder(salesOrderNo) {
    try {
        const query = `SELECT l.SalesOrderNo,
                              h.OrderStatus,
                              IFNULL(h.OrderType, 'C')                AS OrderType,
        ...
        const [row] = rows.map(row => {
            ...
            try {
                ...
                debug("loadLoggedSaleOrder()", err.message, row.SalesOrderNo);
                b2bAction = {error: 'Invalid Action', oldAction: row.b2bAction};
            }
            return {
                ...row,
        ...
        if (err instanceof Error) {
            debug("loadLoggedSaleOrder()", err.message);
            return Promise.reject(err);
        }
        debug("loadLoggedSaleOrder()", err);
        return Promise.reject(new Error('Error in loadLoggedOpenOrders()'));
    }
}
 ...
 * @return {Promise<*>}
 */
async function loadLoggedOrders(params) {
    try {
        ...
        return rows.map(row => {
            ...
            row.DateCreated = dayjs(row.DateCreated).add(row.TimeCreated ?? 8, 'hours').toISOString();
            row.DateUpdated = dayjs(row.DateUpdated).add( row.TimeUpdated ?? 8, 'hours').toISOString();
        ...
        debug("loadLoggedOrders()", err.message);
        return Promise.reject(err);
    }
}

async function loadLoggedOrderHistory(salesOrderNo) {
    try {
        ...
        debug("loadLoggedOrderHistory()", err.message);
        return Promise.reject(err);
    }
}

export const getLoggedOrders = async (req, res) => {
    try {
        const result = await loadLoggedOrders(req.params);
        let history;
        if (req.params.SalesOrderNo && result.length === 1) {
            history = await loadLoggedOrderHistory(req.params.SalesOrderNo)
        }
...
export const getLoggedOpenOrders = async (req, res) => {
    try {
        ...
        const salesOrders = await loadLoggedOpenOrders(req.query);
...
export const getLoggedSalesOrder = async (req, res) => {
    try {
        const salesOrderNo = req.params.salesOrderNo;
        if (!salesOrderNo) {
            return res.status('404').json({error: 'Sales Order not found'});
        }
        let salesOrder = await loadLoggedSaleOrder(salesOrderNo);
        if (!salesOrder) {
            return res.status('404').json({error: 'Sales Order not found'});
        }
        const detail = await loadOrderHistoryDetail(salesOrderNo)
        const history = await loadLoggedOrderHistory(salesOrderNo);
        res.json({
            salesOrder,
            detail,
            history,
...
export const getLoggedOrderHistory = async (req, res) => {
    try {
        const result = await loadLoggedOrderHistory(req.params.SalesOrderNo);
        res.json({result})
    } catch (err) {
...
export const getLoggedOrderUserStats = async (req, res) => {
    try {
