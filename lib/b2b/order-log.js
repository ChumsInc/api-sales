...
function MASTime(Date, Time) {
    const time = Number.parseFloat(Time);
    const seconds = time * 3600;
    const date = addSeconds(parseISO(Date), seconds);
...
async function loadOrders(params) {
    ...
    `;
        ...
        return rows.map(row => {
            ...
            row.DateCreated = MASTime(row.DateCreated, row.TimeCreated);
            row.DateUpdated = MASTime(row.DateUpdated, row.TimeUpdated);
