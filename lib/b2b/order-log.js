...
async function loadOrderUserStats({userId, Company, minDate, maxDate}) {
    try {
        ...
        rows
            ...
            .forEach(row => {
                const {ARDivisionNo, CustomerNo, BillToName, SalesOrderNo, OrderDate, ShipExpireDate,
                    OrderStatus, OrderTotal, UserID, SalespersonDivisionNo, SalespersonNo, SalespersonName} = row;
...
exports.getOrderUserStats = async (req, res) => {
    try {
