...
export async function loadCategoryProductValidation(): Promise<CategoryPageValidationRow[]> {
    try {
        ...
        `;
        const [rows] = await mysql2Pool.query<(CategoryPageValidationRow & RowDataPacket)[]>(sql);
        return rows;
    } catch (err: unknown) {
        if (err instanceof Error) {
...
export async function loadProductCategoryValidation(): Promise<ProductCategoryPageValidationRow[]> {
    try {
        const sql = `SELECT p.products_id                AS id,
                            p.products_keyword           AS keyword,
                            p.products_model             AS ItemCode,
        ...
        const [rows] = await mysql2Pool.query<(ProductCategoryPageValidationRow & RowDataPacket)[]>(sql);
        return rows;
    } catch (err: unknown) {
        if (err instanceof Error) {
