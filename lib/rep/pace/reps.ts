...
export async function loadUserReps(userid: number): Promise<SalespersonRow[]> {
    try {
        const [rows] = await mysql2Pool.query<(SalespersonRow & RowDataPacket)[]>(availableRepsSQL, {userid});
        return rows.map(row => ({
            ...row,
            Active: !!row.Active,
            manager: null,
...
export async function loadUserRep(userid: number): Promise<SalespersonRow | null> {
    try {
        const [rows] = await mysql2Pool.query<(SalespersonRow & RowDataPacket)[]>(userRepsSQL, {userid});
        if (!rows.length) {
            return null;
        }
        const rep = rows[0];
