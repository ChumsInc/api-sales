...
`;
export async function loadManagedCustomers({ Company, SalespersonDivisionNo, SalespersonNo, maxDate, minDate, groupByCustomer }) {
    try {
        ...
        return repCustomers.map(row => ({
            ...
            rate: new Decimal(row.InvPYTD).eq(0)
                ...
                : new Decimal(row.InvCYTD).sub(row.InvPYTD).div(row.InvPYTD).toString(),
            pace: new Decimal(row.InvPY).eq(0)
                ? new Decimal(row.InvCYTD).add(row.OpenOrders).toString()
                : new Decimal(row.rate).add(1).times(row.InvPY).toString()
        }));
    }
    catch (err) {
