...
async function checkCustomers() {
    try {
        ...
        if (err instanceof Error) {
            console.debug("checkCustomers()", err.message);
            return Promise.reject(err);
        }
        console.debug("checkCustomers()", err);
...
async function checkShipToAddresses() {
    try {
        ...
        if (err instanceof Error) {
            console.debug("checkShipToAddresses()", err.message);
            return Promise.reject(err);
        }
        console.debug("checkShipToAddresses()", err);
...
async function checkSalesOrders() {
    try {
        ...
        if (err instanceof Error) {
            console.debug("checkSalesOrders()", err.message);
            return Promise.reject(err);
        }
        console.debug("checkSalesOrders()", err);
    ...
    try {
        ...
        const sql = `SELECT ih.InvoiceType,
                     ...
                     WHERE ih.Company = 'chums'
                       ...
                       AND s.UDF_TERMINATED = 'Y'
                     ORDER BY Company, InvoiceNo, HeaderSeqNo `;
        ...
        if (err instanceof Error) {
            console.debug("checkInvoices()", err.message);
            return Promise.reject(err);
        }
        console.debug("checkInvoices()", err);
        return Promise.reject(new Error('Error in checkInvoices()'));
    }
}

const getTerminatedRepAccounts = async (req, res) => {
    try {
...
const getTerminatedRepOpenOrders = async (req, res) => {
    try {
...
const getTerminatedRepInvoices = async (req, res) => {
    try {
...
const renderTerminatedRepOrdersReport = async (req, res) => {
    try {
...
const renderTerminatedRepInvoiceReport = async (req, res) => {
    try {
        const invoices = await checkInvoices(req.params);
...
exports.getTerminatedRepInvoices = getTerminatedRepInvoices;
exports.getTerminatedRepOpenOrders = getTerminatedRepOpenOrders;
exports.renderTerminatedRepInvoiceReport = renderTerminatedRepInvoiceReport;
exports.renderTerminatedRepOrdersReport = renderTerminatedRepOrdersReport;
exports.getTerminatedRepAccounts = getTerminatedRepAccounts;
