...
export const renderTerminatedRepOrdersReport = async (req, res) => {
    try {
        ...
        res.render('sales/terminated-rep-orders.pug', {orders});
    } catch (err) {
        if (err instanceof Error) {
...
export const renderTerminatedRepOrdersEmail = async (req, res) => {
    try {
        const orders = await checkSalesOrders(req.params);
        if (!orders || !orders.length) {
            res.status(301).send();
        ...
        const html = await new Promise((resolve, reject) => {
            res.render('sales/terminated-rep-orders.pug', {orders}, (err, html) => {
                if (err) {
                    return reject(err);
                }
                return resolve(html);
        ...
        const attachments = [{
            ...
            contentType: 'application/json; name=orders.json',
            encoding: 'base64',
            contentDisposition: 'attachment',
            content: Buffer.from(JSON.stringify(orders, undefined, 2)).toString('base64')
        }]
        res.json({
            html,
            attachments,
