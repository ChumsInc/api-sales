...
const {mysql2Pool, resultToExcelSheet, buildWorkBook, ColumnNames} = require('chums-local-modules');
const {formatISO9075, lastDayOfYear, setDayOfYear, addYears, parseISO} = require('date-fns');
const {loadOpenOrders, loadRecentOrders} = require('./rep-orders');
...
async function loadRepBillToTotal({userid, Company, asOfDate}) {
    try {
        const sql = `SELECT c.SalespersonDivisionNo,
                            c.SalespersonNo,
                            rep.SalespersonName,
                            IFNULL(rep.UDF_TERMINATED, 'N')                         AS TerminatedRep,
                     ...
                     FROM users.user_AR_Customer c
                          ...
                          LEFT JOIN (
                                    ...
                                    WHERE InvoiceDate BETWEEN :CYMinDate AND :CYCurrDate
                          ...
                          LEFT JOIN (
                                    ...
                                    FROM c2.ar_invoicehistoryheader
                                    WHERE InvoiceDate BETWEEN :PYMinDate AND :PYCurrDate
                                    GROUP BY Company, ARDivisionNo, CustomerNo
                                    ) InvPYTD
                          ...
                          LEFT JOIN (
                                    SELECT Company,
                                           ...
                                           SUM(TaxableSalesAmt + NonTaxableSalesAmt - DiscountAmt) AS total
                                    FROM c2.ar_invoicehistoryheader
                                    WHERE InvoiceDate BETWEEN :PYMinDate AND :PYMaxDate
                                    GROUP BY Company, ARDivisionNo, CustomerNo
                                    ) InvPYr
 ...
 * @return {Promise<void>}
 */
const getRepAccounts = async (req, res) => {
    try {
        const params = {
            ...req.params,
...
exports.getRepAccounts = getRepAccounts;
 ...
 * @return {Promise<void>}
 */
const getRepAccountsXLSX = async (req, res) => {
    try {
        const params = {
            ...req.params,
            userid: res.locals?.profile?.user?.id || 0,
...
exports.getRepAccountsXLSX = getRepAccountsXLSX;
 ...
 * @return {Promise<void>}
 */
const getRepTotals = async (req, res) => {
    try {
        const params = {
            ...req.params,
            userid: res.locals?.profile?.user?.id || 0,
...
exports.getRepTotals = getRepTotals;
