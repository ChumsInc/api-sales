...
/**
 *
 * @param {string|Date|undefined} asOfDate
 * @return {{CYCurrDate: string, CYMinDate: string, PYMinDate: string, PYCurrDate: string, PYMaxDate: string}}
 */
...
async function loadRepBillToTotal({userid, asOfDate}) {
    try {
        ...
        const {CYCurrDate} = getDates(asOfDate);
...
async function loadRepShipToTotal({userid, asOfDate}) {
    try {
        const sql = `SELECT c.SalespersonDivisionNo,
                            c.SalespersonNo,
                            rep.SalespersonName,
...
async function loadBillToAccountList({userid, SalespersonNo, asOfDate}) {
    try {
...
async function loadShipToAccountList({userid, SalespersonNo, asOfDate}) {
    try {
        const sql = `SELECT c.ARDivisionNo,
                            c.CustomerNo,
                            c.CustomerName,
...
const billToColumnNames = (asOfDate) => {
    const {CYCurrDate, PYMinDate} = getDates(asOfDate);
