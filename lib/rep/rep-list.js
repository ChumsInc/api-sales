...
const debug = Debug('chums:lib:sales:rep:rep-list');
 ...
 * @return {Promise<*>}
 */
export async function loadRepList({company, userid}) {
    try {
 ...
 * @param {string} company
 * @param {string|number} userid
 * @return {Promise<Salesperson[]>}
 */

export async function loadCondensedRepList({company, userid}) {
    try {
        company = getDBCompany(company);
...
export const getCondensedRepList = async (req, res) => {
    try {
        const userid = res.locals.profile?.user?.id ?? 0;
...
export const getUserRepList = async (req, res) => {
    try {
