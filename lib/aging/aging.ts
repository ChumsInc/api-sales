...
import type {CustomerAging, RepAgingTotal} from "./aging-types.js";
...
export interface LoadRepAgingArgs {
    userId: number;
    salespersonDivisionNo?: string;
    salespersonNo?: string;
}

export async function loadRepAging(args: LoadRepAgingArgs):Promise<RepAgingTotal[]> {
    try {
        const sql = `
        ...
        const [rows] = await mysql2Pool.query<(RepAgingTotal & RowDataPacket)[]>(sql, sqlParams);
        return rows.map(row => ({
            ...row,
...
export async function loadRepCustomerAging(args: LoadRepAgingArgs):Promise<CustomerAging[]> {
    try {
        const sql = `
    ...
    const [rows] = await mysql2Pool.query<(CustomerAging & RowDataPacket)[]>(sql, sqlParams);
        return rows.map(row => ({
            ...row,
            TotalDue: new Decimal(row.CurrentBalance ?? 0)
                .add(row.AgingCategory1 ?? 0)
