...
#                    c.SalespersonNo,
           ...
           IF(IFNULL(:SalesP1, 0) = 0,
              0,
              IFNULL((SELECT ROUND(SUM(h.TaxableSalesAmt + h.NonTaxableSalesAmt - h.DiscountAmt), 2)
           ...
           IF(IFNULL(:SalesP3, 0) = 0,
              0,
              IFNULL((SELECT ROUND(SUM(TaxableSalesAmt + NonTaxableSalesAmt - DiscountAmt), 2)
                      FROM c2.ar_invoicehistoryheader h
                      WHERE h.Company = c.Company
                        AND h.ARDivisionNo = c.ardivisionno
                        AND h.CustomerNo = c.customerno
    ...
    FROM c2.ar_customer c
             ...
             LEFT JOIN (SELECT DISTINCT c.company, c.ARDivisionNo, c.CustomerNo, 'YES' AS isb2b
                        FROM users.users u
                                 INNER JOIN users.accounts a
                                            ON a.userid = u.id
    ...
    WHERE c.Company = 'chums'
      ...
      AND (IFNULL(:includeInactive, '0') = '1' OR c.CustomerStatus = 'A')
      AND (
