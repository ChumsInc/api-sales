...
import {shipToQuery} from './queries.js';
...
async function loadAccounts({filters, periods, sqlSort}: LoadAccountProps): Promise<LoadAccountResult> {
    try {
        ...
        const [rows] = await mysql2Pool.query<AccountRow[]>(query, {
            ...filters, ...periods,
...
const prepForRender = (rows: ALCustomer[]): ALCustomer[] => {
    return rows.map(row => {
        return {
            ...row,
            ...{
...
export const getShipToAccountList = async (req: Request, res: Response<unknown, ValidatedUser>) => {
    try {
        ...
        if (err instanceof Error) {
            debug("getShipToAccountList()", err.message);
            res.status(500).json({error: err.message, name: err.name});
            return;
        }
        res.status(500).json({error: 'unknown error in getShipToAccountList'});
    }

};

export const renderShipToAccountList = async (req: Request, res: Response<unknown, ValidatedUser>) => {
    try {
        ...
        res.status(500).json({error: 'unknown error in renderShipToAccountList'});
    }
}

export const renderShipToAccountListXLSX = async (req: Request, res: Response<unknown, ValidatedUser>) => {
    try {
        const {sqlSort, fields, filters, periods} = await parse(req.body, res.locals.profile!.user.id);
        const {rows} = await loadAccounts({filters, periods, sqlSort});
        const columnNames: Partial<ColumnNames<ALCustomer>> = {};
        ['account', 'ARDivisionNo', 'CustomerNo', 'ShipToCode', ...fields].forEach(field => {
        ...
        if (err instanceof Error) {
            debug("renderShipToAccountListXLSX()", err.message);
            res.status(500).json({error: err.message, name: err.name});
            return;
        }
        res.status(500).json({error: 'unknown error in renderShipToAccountListXLSX'});
    }
};
