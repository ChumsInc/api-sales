...
const queryOrders = `
    ...
    WHERE hh.OrderStatus IN ('A', 'C')
      ...
      AND IF(ifnull(:dateType, '') = 'od', hh.OrderDate BETWEEN :minDate AND :maxDate, hh.ShipExpireDate BETWEEN :minDate AND :maxDate)
    ...
    WHERE soh.OrderType IN ('M')
      ...
      AND IF(ifnull(:dateType, '') = 'od', soh.OrderDate BETWEEN :minDate AND :maxDate, soh.ShipExpireDate BETWEEN :minDate AND :maxDate)
...
async function loadOrderStatusList({Company, dateType, minDate, maxDate, filterDivision, filterRep}) {
    try {
        ...
        return rows.map(row => ({
            ...
            week: getWeek(new Date(row.OrderDate)),
            day: getDayOfYear(new Date(row.OrderDate)),
        }))
    } catch (err) {
