...
const sqlSOHeader = `
    SELECT h.Company,
           ...
           h.OrderStatus,
           h.ShipExpireDate                          AS ShipExpireDate,
           h.ARDivisionNo,
           h.CustomerNo,
           h.ShipToCode,
           h.BillToName,
...
const sqlSODetail = `
    SELECT d.Company,
           ...
           iw.BinLocation,
           i.Category4,
           ifnull(CAST(CEILING(JSON_EXTRACT(pa.casePackDimensions, '$.length')
                              * JSON_EXTRACT(pa.casePackDimensions, '$.width')
                              * JSON_EXTRACT(pa.casePackDimensions, '$.height')
                              / JSON_EXTRACT(pa.casePackDimensions, '$.quantity')
               * o.QuantityOrdered) AS CHAR), i.Volume) AS Volume,
           i.ShipWeight,
           i.ProductType,
           i.ProductLine,
           i.UDF_UPC,
    ...
    FROM c2.SO_SalesOrderHistoryDetail d
             ...
             LEFT JOIN barcodes.bc_customerdetail bcd
                       ON bcd.CustomerID = bcc.id AND
                          bcd.ItemNumber = d.ItemCode
             LEFT JOIN c2.PM_Products p
                       ON p.SKU = i.Category4
             LEFT JOIN c2.PM_ProductAttributes pa
                       ON pa.productId = p.id
    WHERE d.Company = :company
      AND d.SalesOrderNo = :salesOrderNo
      AND IF(d.ItemType = 1, d.QuantityOrderedRevised <> 0, 1)
    ORDER BY LineSeqNo
`;
...
async function loadOpenSalesOrder({company, salesOrderNo}) {
    try {
        ...
        if (headerRows.length === 0) {
            ...
            const res = await apiFetchJSON(`https://intranet.chums.com/node-sage/api/${getSageCompany(company)}/salesorder/${encodeURIComponent(salesOrderNo)}`);
