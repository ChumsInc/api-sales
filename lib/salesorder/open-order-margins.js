...
const queryOrdersList = `
    ...
    FROM c2.SO_SalesOrderHeader h
             INNER JOIN (SELECT h.Company,
                                ...
                                COUNT(d.LineSeqNo) =
                                SUM(salesGL.MainAccountCode IN (:promoGLCodes)
                                    OR cogsGL.MainAccountCode IN
                                       (:promoGLCodes)
                         ...
                         WHERE h.OrderType NOT IN ('Q', 'M')
                           ...
                           AND (IFNULL(:filterPromo, '0') = '0' OR
                                salesGL.MainAccountCode NOT IN (:promoGLCodes))
                           AND (IFNULL(:filterPromo, '0') = '0' OR
                                cogsGL.MainAccountCode NOT IN (:promoGLCodes))
