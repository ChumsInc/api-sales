...
const queryOrdersList = `
    SELECT DISTINCT h.Company,
                    ...
                    (select id FROM users.users u where u.id = ifnull(d.b2bPromotedBy, d.b2bCreatedBy)) as b2bUserID,
                    (select name FROM users.users u where u.id = ifnull(d.b2bPromotedBy, d.b2bCreatedBy)) as b2bUserName,
                    (select company FROM users.users u where u.id = ifnull(d.b2bPromotedBy, d.b2bCreatedBy)) as B2BCompany,
    ...
    FROM c2.SO_SalesOrderHeader h
             INNER JOIN (SELECT h.Company,
                                ...
                                COUNT(d.LineSeqNo) =
                                SUM(salesGL.MainAccountCode IN (SELECT MainAccountCode FROM c2.SO_OrderMarginGLAccounts where Company = h.Company)
                                    OR cogsGL.MainAccountCode IN (SELECT MainAccountCode FROM c2.SO_OrderMarginGLAccounts where Company = h.Company)
                                    OR ifnull(omc.isPromo, 0))                             AS isPromo,
                                (select l.UserID
                                 FROM b2b.SalesOrderLog l
                                          inner join users.users u on u.id = l.UserID
                                 where l.dbCompany = h.Company and l.SalesOrderNo = h.SalesOrderNo and  json_extract(l.action, '$.action') = 'new') as b2bCreatedBy,
                                (select l.UserID
                                 FROM b2b.SalesOrderLog l
                                          inner join users.users u on u.id = l.UserID
                                 where l.dbCompany = h.Company and l.SalesOrderNo = h.SalesOrderNo and  json_extract(l.action, '$.action') = 'promote') as b2bPromotedBy
                         FROM c2.SO_SalesOrderHeader h
                         ...
                         WHERE h.OrderType NOT IN ('Q', 'M')
                           ...
                           AND (IFNULL(:filterPromo, '0') = '0' OR
                                salesGL.MainAccountCode NOT IN (SELECT MainAccountCode FROM c2.SO_OrderMarginGLAccounts where Company = h.Company))
                           AND (IFNULL(:filterPromo, '0') = '0' OR
                                cogsGL.MainAccountCode NOT IN (SELECT MainAccountCode FROM c2.SO_OrderMarginGLAccounts where Company = h.Company))
