...
import {PaceCustomerRow, PaceDivisionParams} from "./pace-types.js";
import {RowDataPacket} from "mysql2";

const debug = Debug('chums:api:sales:pace:division');

export async function loadPaceByCustomer({year, month, ARDivisionNo}:PaceDivisionParams):Promise<PaceCustomerRow[]> {
    try {
        const sql = `
            ...
            FROM c2.ar_customer d
                     ...
                     LEFT JOIN (SELECT c.Company,
                                ...
                                WHERE c.Company = 'chums'
                                  ...
                                  AND ih.TransactionDate BETWEEN :minDate AND :maxDate
                                GROUP BY c.Company,
                                         c.ARDivisionNo,
        ...
        const [rows] = await mysql2Pool.query<(PaceCustomerRow & RowDataPacket)[]>(sql, {minDate, maxDate, ARDivisionNo});
        return rows;
    } catch (err) {
