 ...
 */
async function loadPaceBySegment({company, year, month, ARDivisionNo}) {
    try {
        ...
        const sql = `
            SELECT d.ARDivisionNo,
                   ...
                   IFNULL(invoiced.InvoiceTotal, 0) + IFNULL(current.InvoiceTotal, 0) + IFNULL(prevOpen.PrevOrderTotal, 0)
            ...
            FROM (
                     SELECT c.Company,
                            ...
                            (
                                ...
                                WHERE ctt.CustomerType = IFNULL(IFNULL(ct.ReportAsType, c.CustomerType), 'NONE')
                                )                                                  AS Description,
                            COUNT(*)                                               AS Customers
                     ...
                     GROUP BY ARDivisionNo, IFNULL(IFNULL(ct.ReportAsType, c.CustomerType), 'NONE')
                     ) d
                 LEFT JOIN (
                SELECT c.Company,
                       c.ARDivisionNo,
                ...
                FROM c2.ar_customer c
                     INNER JOIN c2.ar_invoicehistoryheader ih
                                ON ih.Company = c.Company
                                    AND ih.ARDivisionNo = c.ARDivisionNo AND ih.CustomerNo = c.CustomerNo
                     LEFT JOIN c2.AR_CustomerType ct
                ...
                GROUP BY c.Company,
                         c.ARDivisionNo,
                         IFNULL(IFNULL(ct.ReportAsType, c.CustomerType), 'NONE')
                ) invoiced
                ...
                WHERE c.Company = :company
                  AND ih.InvoiceType <> 'XD'
                  AND ih.InvoiceDate BETWEEN :minDate AND :maxDate
                GROUP BY c.Company, c.ARDivisionNo, IFNULL(IFNULL(ct.ReportAsType, c.CustomerType), 'NONE')
                ) current
        ...
        `;
        const args = {company, year, month, minDate, maxDate, ARDivisionNo}
...
exports.loadPaceBySegment = loadPaceBySegment;
