...
import {PaceDivisionRow, PaceParams} from "./pace-types.js";
...
export async function loadPaceByDivision({year, month}:PaceParams):Promise<PaceDivisionRow[]> {
    try {
        ...
        `;
        const args = {year, month, minDate, maxDate}
        const [rows] = await mysql2Pool.query<(PaceDivisionRow & RowDataPacket)[]>(sql, args);
        return rows;
    } catch (err) {
