...
const deleteOnExisting:string[] = [
    'c2.ar_customer_location',
    'c2.AR_CustomerContact',
    'c2.AR_CustomerCreditCard',
]

export interface CheckExistingTables {
    [key:string]: number;
}
export interface CheckExistingResponse {
    [key: string]: CheckExistingTables;
}

export interface CheckExistingRows {
    remaining: number;
}

type CheckExistingRowsResponse = CheckExistingRows & RowDataPacket;
...
async function checkExistingRows() {
    try {
        ...
        const response:CheckExistingResponse = {};
        for await(const c of customers.slice(0, 10)) {
            ...
            for await (const t of tables) {
                ...
                const [rows] = await mysql2Pool.query<CheckExistingRowsResponse[]>(_sql, {arDivisionNo, customerNo});
...
export const getExistingOpticalRows = async (req:Request, res:Response) => {
    try {
...
async function checkARCustomerSalesHistory(customerNo:string) {
    const sql = `SELECT *
