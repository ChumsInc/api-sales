import {
    ChangedRowResponse,
    ErrorResponse,
    RemainingRowsResponse,
    TableDefinition, TableHandlerOptions,
    TableHandlerResponse
} from "./types.js";
...
const emptyResponse: ChangedRowResponse = {
    affectedRows: 0,
    remaining: 0,
}
...
export async function execRenumberCustomer(userId: number, from: string, to: string, options: TableHandlerOptions): Promise<(TableHandlerResponse|ErrorResponse)[]> {
    try {
...
async function checkExisting(table: string, from: string): Promise<number> {
    try {
        ...
        const [rows] = await mysql2Pool.query<RemainingRowsResponse[]>(sql, {arDivisionNo, customerNo});
...
async function updateCustomer(table: string, from: string, to: string): Promise<number> {
    try {
        ...
        const [result] = await mysql2Pool.query<ResultSetHeader>(sql, {
            arDivisionNo,
            customerNo,
            newARDivisionNo,
            newCustomerNo
...
async function genericTableHandler(userId: number, table: string, from: string, to: string, options:TableHandlerOptions): Promise<TableHandlerResponse> {
    try {
        const start = new Date().valueOf();
        const response:TableHandlerResponse = {
            table,
            ...emptyResponse
        };
...
export async function deleteRemainingHandler(userId: number, table: string, from: string, to: string, options: TableHandlerOptions): Promise<TableHandlerResponse> {
    try {
        const start = new Date().valueOf();
...
export async function ARCustomerSalesHistory(userId: number, table: string, from: string, to: string, options:TableHandlerOptions): Promise<TableHandlerResponse> {
    try {
        const start = new Date().valueOf();
        const response = await genericTableHandler(userId, table, from, to, {...options, deferLogging: true});
        if (options.dryRun) {
...
export async function ARAlternativeInvoiceHandler(userId: number, table: string, from: string, to: string, options:TableHandlerOptions): Promise<TableHandlerResponse> {
    try {
        const start = new Date().valueOf();
        const response = await genericTableHandler(userId, table, from, to, {...options, deferLogging: true});
        if (options.dryRun) {
