...
const debug = require('debug')('chums:lib:gdpr');

async function loadSalesOrder({company, salesOrderNo}) {
    try {
        const sqlSO = `SELECT SalesOrderNo, BillToName, BillToAddress1, ShipToName, ShipToAddress1, EmailAddress FROM c2.SO_SalesOrderHeader where company = :company and SalesOrderNo = :salesOrderNo`
        const sqlInv = `SELECT InvoiceNo, BillToName, BillToAddress1, ShipToName, ShipToAddress1, EmailAddress FROM c2.ar_invoicehistoryheader where company = :company and SalesOrderNo = :salesOrderNo`
    ...
    try {
        ...
        const sqlSOHistoryHeader = `
            ...
            SET BillToName     = 'REMOVED PII',
                ...
                EmailAddress   = 'removed@domain.tld'
        ...
        const [SO_SalesOrderHistoryHeader] = await mysql2Pool.query(sqlSOHistoryHeader, args);
        const [ar_invoicehistoryheader] = await mysql2Pool.query(sqlARInvHistoryHeader, args);
        const [shopify_fulfillment] = await mysql2Pool.query(sqlShopifyFulfillment, args);
        const [shopify_orders] = await mysql2Pool.query(sqlShopifyOrders, args);

        return {args, SO_SalesOrderHistoryHeader, rARH: ar_invoicehistoryheader, shopify_fulfillment, shopify_orders}
    } catch (err) {
...
async function execGDPRRequest(req, res) {
    try {
...
async function getGDPRSORequest(req, res) {
    try {
...
async function execGDPRSORequest(req, res) {
    try {
...
exports.execGDPRRequest = execGDPRRequest;
exports.execGDPRSORequest = execGDPRSORequest;
exports.getGDPRSORequest = getGDPRSORequest;
