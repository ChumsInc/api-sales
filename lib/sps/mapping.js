...
async function loadRecentCSV({id, Company, ARDivisionNo, CustomerNo}) {
    try {
        const sql = `SELECT ih.Company,
                            ih.ARDivisionNo,
                            ih.CustomerNo,
                            ih.CustomerPONo,
                            ih.timestamp,
                            ...
                            il.data
                     FROM sps_edi.customers c
                          INNER JOIN sps_edi.importHistory ih
                                     ON ih.dbCompany = c.Company
                                         AND ih.ARDivisionNo = c.ARDivisionNo
                                         AND ih.CustomerNo = c.CustomerNo
                          LEFT JOIN  sps_edi.csvImportLog il
                                     ON il.Company = ih.Company
                                         AND il.ARDivisionNo = ih.ARDivisionNo
                                         AND il.CustomerNo = ih.CustomerNo
                                         AND il.CustomerPONo = ih.CustomerPONo
                     WHERE (IFNULL(:id, 0) = c.id OR
                            (c.Company = :Company AND c.ARDivisionNo = :ARDivisionNo AND c.CustomerNo = :CustomerNo)
                               )
                     ORDER BY ih.id DESC
                     LIMIT 5`
        const args = {id, Company, ARDivisionNo, CustomerNo};
        const [rows] = await mysql2Pool.query(sql, args);
        return rows;
    } catch (err) {
        debug("getRecentCSV()", err.message);
        return Promise.reject(err);
    }
}
...
exports.getCustomerRecentCSV = async (req, res) => {
    try {
        const orders = await loadRecentCSV(req.params);
        res.json({orders});
    } catch(err) {
        debug("getCustomerRecentCSV()", err.message);
        return Promise.reject(err);
    }
}
