...
const debug = Debug('chums:lib:sales:pricing');
...
const loadPriceLevels = async (priceLevel) => {
    try {
        const query = `
            ...
            FROM (SELECT DISTINCT Company, CustomerPriceLevel AS PriceLevel
                  FROM c2.im_pricecode
                  WHERE im_pricecode.PriceCodeRecord = 0
                  ...
                  WHERE CustomerStatus = 'A'
                    and Company = 'chums'
                    AND PriceLevel IS NOT NULL) pc
                     LEFT JOIN c2.IM_PriceLevels pl
                               ON c2.dbCompany(pl.Company) = pc.Company AND pl.PriceLevel = pc.PriceLevel
                     LEFT JOIN (SELECT Company, PriceLevel, COUNT(*) AS customers
                                FROM c2.ar_customer
                                WHERE CustomerStatus = 'A'
                                GROUP BY Company, PriceLevel) c
                               ON c.Company = pc.Company AND c.PriceLevel = pc.PriceLevel
                     LEFT JOIN (SELECT pc.Company, pc.CustomerPriceLevel, COUNT(DISTINCT pc.PriceCode) AS priceCodes
                                ...
                                GROUP BY Company, CustomerPriceLevel) pcc
            ...
            FROM c2.IM_PriceLevels pl
                     LEFT JOIN (SELECT Company, PriceLevel, COUNT(*) AS customers
                                ...
                                WHERE CustomerStatus = 'A'
                                GROUP BY Company, PriceLevel) c
                               ON c.Company = c2.dbCompany(pl.Company) AND c.PriceLevel = pl.PriceLevel
                     LEFT JOIN (SELECT pc.Company, pc.CustomerPriceLevel, COUNT(DISTINCT pc.PriceCode) AS priceCodes
                                FROM c2.im_pricecode pc
                                         INNER JOIN c2.ci_item i
                                                    ON i.company = pc.Company AND i.PriceCode = pc.PriceCode
                                WHERE i.ProductType <> 'D'
                                  ...
                                  AND pc.PriceCodeRecord = 0
                                GROUP BY Company, CustomerPriceLevel) pcc
                               ON pcc.Company = c2.dbCompany(pl.Company) AND pcc.CustomerPriceLevel = pl.PriceLevel
    ...
    try {
        const sql = `SELECT DISTINCT p.Company,
                     ...
                     FROM c2.im_pricecode p
                              INNER JOIN (SELECT Company, PriceCode, COUNT(*) AS ct
                                          FROM c2.ci_item
                                          WHERE ProductType <> 'D'
                                            AND InactiveItem <> 'Y'
                                          GROUP BY Company, PriceCode) ip
    ...
    try {
        const sql = `SELECT DISTINCT p.Company,
                     ...
                     FROM c2.im_pricecode p
                              ...
                              LEFT JOIN (SELECT DISTINCT company, PriceCode
                                         FROM c2.ci_item
    ...
    try {
        const query = `
            SELECT c2.dbCompany(w.company)                                       AS Company,
                   ...
                   w.PricingMethod,
                   w.CustomerPriceLevel,
                   IFNULL(p.BreakQuantity1, '99999999')                          AS BreakQuantity1,
                   IFNULL(p.DiscountMarkup1, 0)                                  AS DiscountMarkup1,
                   w.DiscountMarkup1                                             AS newDiscountMarkup1,
                   w.UserName,
                   w.timestamp,
...
export const getPriceChangeUsers = async (req, res) => {
    try {
...
export const getUserPriceChanges = async (req, res) => {
    try {
...
export const getUserChangesImport = async (req, res) => {
    try {
...
export const getPriceCode = async (req, res) => {
    try {
...
export const postNewPricing = async (req, res) => {
    try {
...
export const delNewPricingEntry = async (req, res) => {
    try {
...
export const getPriceLevels = async (req, res) => {
    try {
...
export const getPriceLevel = async (req, res) => {
    try {
...
export const postPriceLevel = async (req, res) => {
    try {
...
export const postPriceLevelSort = async (req, res) => {
    try {
        const body = req.body;
...
export const getPriceCodes = async (req, res) => {
    try {
