    ...
    try {
        const sql = `
            ...
            FROM c2.ar_invoicehistoryheader h
                     LEFT JOIN (SELECT Company, InvoiceNo, HeaderSeqNo, SUM(t.Weight) AS Weight
                                FROM c2.AR_InvoiceHistoryTracking t
                                GROUP BY Company, InvoiceNo, HeaderSeqNo) it
        ...
        rows.forEach(row => {
            row.SHIPMENT_DATE = dayjs(row.SHIPMENT_DATE).format('MM/DD/YYYY');
...
export const getCensusAudit = async (req, res) => {
    try {
        const params = {
            minDate: req.query.minDate ?? req.params.minDate,
            maxDate: req.query.maxDate ?? req.params.maxDate,
        }
        const rows = await loadAuditData(params);
        res.json(rows);
    } catch (err) {
        if (err instanceof Error) {
...
export const getCensusAuditXLSX = async (req, res) => {
    try {
        const params = {
            minDate: req.query.minDate ?? req.params.minDate,
            maxDate: req.query.maxDate ?? req.params.maxDate,
        ...
        const filename = `census-audit--${params.minDate}_${params.maxDate}.xlsx`;
